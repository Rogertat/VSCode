...
(function () {
  ...
  /* ---- mini-Roger: the page companion --------------------------------- */
  var comp = document.querySelector("[data-companion]");
  if (comp) {
    var compOff = false;
    try { compOff = sessionStorage.getItem("companionOff") === "1"; } catch (e) {}
    if (compOff) {
      comp.remove();
    } else {
      comp.classList.add("is-on");
      var bubble = comp.querySelector(".companion-bubble");
      var fig = comp.querySelector("[data-companion-poke]");
      var bubbleTimer = null;
      var asleep = false;

      var speak = function (text, holdMs) {
        bubble.textContent = text;
        bubble.classList.add("show");
        if (bubbleTimer) clearTimeout(bubbleTimer);
        if (holdMs !== 0) {
          bubbleTimer = setTimeout(function () { bubble.classList.remove("show"); }, holdMs || 4000);
        }
      };
      var wake = function () {
        if (!asleep) return;
      ...
      if (!prefersReducedMotion) {
        var blinkLoop = function () {
          setTimeout(function () {
            comp.classList.add("is-blink");
            setTimeout(function () {
              comp.classList.remove("is-blink");
              blinkLoop();
            }, 150);
      ...
      var pupils = comp.querySelectorAll(".pupil");
      if (pupils.length && window.matchMedia("(pointer: fine)").matches && !prefersReducedMotion) {
        var compTick = false;
        document.addEventListener("mousemove", function (e) {
          if (compTick || asleep) return;
          compTick = true;
          window.requestAnimationFrame(function () {
            compTick = false;
            ...
            var d = Math.sqrt(dx * dx + dy * dy) || 1;
            var reach = Math.min(2.6, d / 60);
            pupils.forEach(function (p) {
              p.style.transform = "translate(" + ((dx / d) * reach).toFixed(2) + "px," + ((dy / d) * reach).toFixed(2) + "px)";
            });
          });
        });
      }

      // wave hello on arrival
      if (!prefersReducedMotion) {
        setTimeout(function () {
          comp.classList.add("is-wave");
          speak("hi! i'm mini-Roger. the big one is better at Python.", 4600);
          setTimeout(function () { comp.classList.remove("is-wave"); }, 1700);
        }, 1100);
      }

      // a quip per section as you scroll
      var LINES = {
        about: "everything in here is true. i fact-checked.",
        skills: "he actually uses all of these. daily.",
        voices: "my training data too, honestly.",
        experience: "6 companies, 0 dull days.",
        contact: "psst - run the oracle. it never lies."
      };
      var lastQuip = "";
      var quipSections = Object.keys(LINES)
        .map(function (id) { return document.getElementById(id); })
        .filter(Boolean);
      window.addEventListener("scroll", function () {
        if (asleep || prefersReducedMotion) return;
        var line = window.scrollY + window.innerHeight * 0.5;
        var cur = "";
        quipSections.forEach(function (s) { if (s.offsetTop <= line) cur = s.id; });
        if (cur && cur !== lastQuip) {
          lastQuip = cur;
          speak(LINES[cur]);
        }
      ...
      // poke him
      var POKES = [
        "careful - i bruise like a mango.",
        "try dark mode. i look great in it.",
        "click around the hero. the neurons fire!",
        "loss is going down. i can feel it.",
        "hire the big version of me."
      ];
      fig.addEventListener("click", function () {
        if (asleep) { wake(); return; }
        comp.classList.remove("is-hop");
        void comp.offsetWidth; /* restart the hop animation */
        comp.classList.add("is-hop");
        speak(POKES[Math.floor(Math.random() * POKES.length)]);
      });

      // nap when the visitor idles
      var idleTimer = null;
      var restIdle = function () {
        if (asleep) wake();
        if (idleTimer) clearTimeout(idleTimer);
        if (prefersReducedMotion) return;
        idleTimer = setTimeout(function () {
          asleep = true;
          comp.classList.add("is-asleep");
          speak("z z z ...", 0);
        }, 24000);
      };
      ["mousemove", "scroll", "keydown", "touchstart"].forEach(function (ev) {
        window.addEventListener(ev, restIdle, { passive: true });
      });
      restIdle();

      // dismiss for this session
      comp.querySelector("[data-companion-close]").addEventListener("click", function () {
        try { sessionStorage.setItem("companionOff", "1"); } catch (e) {}
        comp.remove();
      });
    }
  }
